...
def DR_loss(W_prototype, feat):
    feat_normal = feat / torch.norm(feat, p=2, dim=1, keepdim=True)
    dot = torch.sum(W_prototype @ feat_normal, dim=1)
    # h_norm2 = torch.ones_like(dot)
    # m_norm2 = torch.ones_like(dot)

    loss = 0.5 * torch.mean((dot - torch.eye(dot.shape[0]).cuda()) ** 2)

    # Calculate cosine similarity for confidence
...
def kl_divergence(p, q):
    ...
    p = torch.clamp(p, min = 0.00001)
    q = torch.clamp(q, min = 0.00001)

    # Check for NaN after adding epsilon
    if torch.isnan(p).any() or torch.isnan(q).any():
        raise ValueError("Input distributions contain NaN values.")

    # Compute KL divergence
    kl = torch.mean(p * (torch.log(p) - torch.log(q)))

    # Check for NaN after the KL divergence computation
    if torch.isnan(kl):
