...
class MYNET(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        # self.virtual_class_fc = nn.Linear(self.num_features, 1, bias=False)
        ...
        self.noise_fc = nn.ModuleList()
        for _ in range(args.base_class):
            self.noise_fc.append(nn.Linear(self.num_features, self.num_features))
    ...
    def cov_loss(self,x,index):
        ...
        for i in range(x.shape[0]):
            ...
            temp.append( self.noise_fc[index[i]](x[i]) )
    ...
    def encode(self, x):
        x = self.encoder(x) # 256 512 7 7
        x = F.adaptive_avg_pool2d(x, 1)
        concat_feature = x.squeeze(-1).squeeze(-1)
